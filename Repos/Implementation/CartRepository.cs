using CoolShopFlowers.Models;
using CoolShopFlowers.Repos.Abstract;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;

namespace CoolShopFlowers.Repos.Implementation
{
    public class CartRepository : ICartRepository
    {
        private readonly DBContext _db;
        private readonly UserManager<User> _userManager;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public CartRepository(DBContext db, IHttpContextAccessor httpContextAccessor,
            UserManager<User> userManager)
        {
            _db = db;
            _userManager = userManager;
            _httpContextAccessor = httpContextAccessor;
        }

        public async Task<int> AddItem(int Id, int qty, string userId)
        {
            using var transaction = await _db.Database.BeginTransactionAsync();
            try
            {
                if (string.IsNullOrEmpty(userId))
                    throw new Exception("User is not logged in");

                var cart = await GetCart(userId);

                if (cart is null)
                {
                    cart = new ShoppingCart
                    {
                        UserId = userId
                    };
                    _db.ShoppingCarts.Add(cart);
                }

                await _db.SaveChangesAsync();

                // Додаємо товар до кошика
                var cartItem = await _db.CartDetails
                                         .FirstOrDefaultAsync(a => a.ShoppingCartId == cart.Id && a.FlowerId == Id);

                if (cartItem != null)
                {
                    // Якщо товар вже є, збільшуємо кількість
                    cartItem.Quantity += qty;
                }
                else
                {
                    // Якщо товару немає в кошику, додаємо новий
                    var flower = await _db.Flowers.FindAsync(Id);
                    if (flower == null) throw new Exception("Flower not found");

                    cartItem = new CartDetail
                    {
                        ShoppingCartId = cart.Id,
                        FlowerId = Id,
                        Quantity = qty,
                        Price = flower.Price
                    };
                    _db.CartDetails.Add(cartItem);
                }

                // Зберігаємо зміни
                await _db.SaveChangesAsync();

                // Підтверджуємо транзакцію
                await transaction.CommitAsync();
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync();
                throw;
            }

            // Повертаємо кількість товарів в кошику
            var cartItemCount = await GetCartItemCount(userId);
            return cartItemCount;
        }

        public async Task<int> RemoveItem(int Id)
        {
            //using var transaction = _db.Database.BeginTransaction();
            string userId = GetUserId();
            try
            {
                if (string.IsNullOrEmpty(userId))
                    throw new Exception("user is not logged-in");
                var cart = await GetCart(userId);
                if (cart is null)
                    throw new Exception("Invalid cart");
                // cart detail section
                var cartItem = _db.CartDetails
                                  .FirstOrDefault(a => a.ShoppingCartId == cart.Id && a.FlowerId == Id);
                if (cartItem is null)
                    throw new Exception("Not items in cart");
                else if (cartItem.Quantity == 1)
                    _db.CartDetails.Remove(cartItem);
                else
                    cartItem.Quantity = cartItem.Quantity - 1;
                _db.SaveChanges();
            }
            catch (Exception ex)
            {

            }
            var cartItemCount = await GetCartItemCount(userId);
            return cartItemCount;
        }
        public async Task<ShoppingCart> GetUserCart()
        {
            var userId = GetUserId();
            if (userId == null)
                throw new Exception("Invalid userid");
            var shoppingCart = await _db.ShoppingCarts
                                  .Include(a => a.CartDetails)
                                  .ThenInclude(a => a.Flower)
                                  .ThenInclude(a => a.Category)
                                  .Where(a => a.UserId == userId).FirstOrDefaultAsync();
            return shoppingCart;

        }
        public async Task<ShoppingCart> GetCart(string userId)
        {
            var cart = await _db.ShoppingCarts.FirstOrDefaultAsync(x => x.UserId == userId);
            return cart;
        }

        public async Task<int> GetCartItemCount(string userId = "")
        {
            if (!string.IsNullOrEmpty(userId))
            {
                userId = GetUserId();
            }
            var data = await (from cart in _db.ShoppingCarts
                              join cartDetail in _db.CartDetails
                              on cart.Id equals cartDetail.ShoppingCartId
                              select new { cartDetail.Id }
                        ).ToListAsync();
            return data.Count;
        }

        private string GetUserId()
        {
            var principal = _httpContextAccessor.HttpContext.User;
            string userId = _userManager.GetUserId(principal);
            return userId;
        }

        public async Task<List<Order>> GetUserOrders()
        {
            var userId = GetUserId();
            if (string.IsNullOrEmpty(userId))
                throw new Exception("User is not logged-in");

            return await _db.Orders
                .Include(o => o.OrderDetail)
                .ThenInclude(od => od.Flower)
                .Where(o => o.UserId == userId && !o.IsDeleted)
                .ToListAsync();
        }

        public async Task<Order?> GetOrderById(int orderId)
        {
            return await _db.Orders
                .Include(o => o.OrderDetail)
                .ThenInclude(od => od.Flower)
                .FirstOrDefaultAsync(o => o.OrderId == orderId && !o.IsDeleted);
        }

        public async Task<bool> DoCheckout(string userId)
        {
            using var transaction = await _db.Database.BeginTransactionAsync();
            try
            {
                if (string.IsNullOrEmpty(userId))
                {
                    throw new Exception("User is not logged in");
                }

                // Отримуємо кошик користувача
                var cart = await GetCart(userId);
                if (cart == null)
                {
                    throw new Exception("Cart not found");
                }

                // Отримуємо товари в кошику
                var cartDetail = _db.CartDetails
                    .Where(a => a.ShoppingCartId == cart.Id).ToList();

                if (cartDetail.Count == 0)
                {
                    throw new Exception("Cart is empty");
                }

                // Створюємо нове замовлення
                var order = new Order
                {
                    OrderId = DateTime.UtcNow.GetHashCode(),
                    UserId = userId,
                    User = await _userManager.FindByIdAsync(userId),
                    CreateDate = DateTime.UtcNow,
                    OrderStatusId = 1,  
                    TotalAmount = (decimal)cartDetail.Sum(cd => cd.Quantity * cd.Price)
                };

                // Додаємо замовлення до бази даних
                _db.Orders.Add(order);
                await _db.SaveChangesAsync();

                // Додаємо деталі замовлення
                foreach (var item in cartDetail)
                {
                    var orderDetail = new OrderDetail
                    {
                        OrderId = order.OrderId,
                        FlowerId = item.FlowerId,
                        Quantity = item.Quantity,
                        Price = item.Price
                    };
                    _db.OrderDetails.Add(orderDetail);
                }

                // Зберігаємо зміни в деталях замовлення
                await _db.SaveChangesAsync();

                // Очищаємо кошик після оформлення замовлення
                _db.CartDetails.RemoveRange(cartDetail);
                await _db.SaveChangesAsync();

                // Підтверджуємо транзакцію
                await transaction.CommitAsync();

                return true;
            }
            catch (Exception ex)
            {
                // Якщо сталася помилка, скасовуємо транзакцію
                await transaction.RollbackAsync();
                Console.WriteLine($"Error during checkout for user {userId}: {ex.Message}");
                return false;
            }
        }

    }
}
